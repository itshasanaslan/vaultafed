...
namespace App2.Views
{
    ...
    public partial class ForgotPasswordPage : ContentPage
    {
       ...
       private async void SendCode(object sender, EventArgs e)
        {
            ...
            if (!UserDatabaseController.ValidateMail(EntryUsername.Text) || EntryUsername.Text == null)
            {
            ...
            user.eMail = EntryUsername.Text.ToLower();

            BtnSend.IsEnabled = false;
            ActivitySpinner.IsRunning = true;

            await Task.Factory.StartNew(() => {
                checkUserResponse = App.databaseController.Operate("CheckUserExists", user);
            });
            ...
            else
            {
                ...
                {
                    await Task.Factory.StartNew(() => {
                        checkUserResponse = App.databaseController.Operate("SendMailCode", user);
       ...
       private async void VerifyCode(object sender, EventArgs e)
        {
            user.password = EntryUsername.Text; // sending code to server as a password.
            ...
            await Task.Factory.StartNew(
                () => {
                    codeResponse = App.databaseController.Operate("VerifyMailCode", user);
                });

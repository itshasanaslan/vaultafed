...
namespace App2.Views.ProfilePages
{
    ...
    public partial class NewPasswordPage : ContentPage
    {
        ...
        if (EntryPassword.Text != EntryPasswordAgain.Text)
            ...
            {
                await DisplayAlert("Connection", "Internet connection error: " + user.GetName(), "OK");
                ManageLoadingAnimation(false);
                return;
            }

            user.SetPassword(EntryPassword.Text);
            user.SetAuthCode(App.session.AuthorizationCode);

            await Task.Factory.StartNew(
            () =>
            {
               
                updateResponse =  App.databaseController.UpdateUser(user);
            }
            );
